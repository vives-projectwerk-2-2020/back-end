...
class Measurement
{
    ...
    public static function find($id, $period, $properties)
    {
        ...
        if ($period == "all") {
            ...
            $errorMessage = array("ERROR"=>"400 Invalid period");
        }
        ...
        if ($validProperties) {
            $errorMessage = array("ERROR"=>"400 Invalid properties");
        }

        $query = "select $meanProperties,sensor_id FROM sensors WHERE sensor_id =~ /$id/ 
        ...
        if ($errorMessage == "" &&  empty($decoded)) {
            $errorMessage = array("ERROR"=>"400 Invalid id");
        }
